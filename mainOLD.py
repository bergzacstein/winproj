...
image = skio.imread('images/build.jpg')
...
img_prewittx = cv2.filter2D(clustered_image, -1, kernelx)
#img_prewitty = cv2.filter2D(equ, -1, kernely)
lazy_imshow(img_prewittx)


kernel = np.ones((5,5),np.uint8)
opening = cv2.morphologyEx(img_prewittx, cv2.MORPH_OPEN, kernel)
lazy_imshow(opening)
#im = scipy.signal.convolve2d(opening,np.ones((10,10)))
#lazy_imshow(im)

sumx = opening.sum(axis=1)
